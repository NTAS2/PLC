...
if ( -e $pidfile) { 
	...
	if( (time-$timest)>3600 ) {
		...
		#OK
		print "Process exist, check $pidfile ?\n";
...
switch ($host) {
	...
	case 'CUSTOM' { print Timestamp(). "CUSTOM: $platform $host \n";$server_port=1579;}
		...
		{ 
		    ...
		    $host = "CUSTOM";
		    $platform = hostname();
		    print "Error when checking platform, unknown type $platform.\n";
		    ##exit(1);
		}
}
...
print Timestamp(). "Syslog sent.\n";	
print Timestamp(). "$server_port ----- $platform \n";
...
sub waitforconfig {
	...
	print "Waiting $SAC to give me $str.\n";
	$SAC->autoflush(1);
	line: while ($line = <$SAC>) {
	    print Timestamp() . "***********************************\n";
